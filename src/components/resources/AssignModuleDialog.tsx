...
interface AssignModuleDialogProps {
  ...
  onAssign: (moduleId: string, clientIds: string[], notes?: string) => Promise<void>;
}
...
export function AssignModuleDialog({
  ...
  const handleSubmit = async () => {
    ...
    try {
      await onAssign(module.id, selectedClientIds, notes || undefined);
      toast({
        title: "Module assigned",
        description: `Successfully assigned to ${selectedClientIds.length} client(s).`,
      });
      onOpenChange(false);
    } catch (error) {
      toast({
        title: "Error",
        description: "Failed to assign module to clients.",
        variant: "destructive",
      });
    } finally {
      ...
      <DialogContent className="sm:max-w-[600px] max-h-[90vh] flex flex-col">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Assign Module to Clients
          </DialogTitle>
          <DialogDescription>
            Select clients to give them access to "{module.name}".
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="flex-1 space-y-4 overflow-hidden">
          {isLoadingClients ? (
            ...
            <Alert>
              ...
              <AlertDescription>
                No clients found. Create clients first to assign modules.
              </AlertDescription>
            </Alert>
          ) : (
            <>
